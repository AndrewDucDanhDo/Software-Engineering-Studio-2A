...
function initExports(revision, mostRecentStats, obsIsAnyOverlayShowing) {
	...
	(() => {
		const exportButton = /** @type {!HTMLButtonElement} */ document.getElementById(
			"export-button"
		);
		const exportOverlay = /** @type {!HTMLDivElement} */ document.getElementById(
			"export-overlay"
		);
		const exportDiv = /** @type {HTMLDivElement} */ document.getElementById(
			"export-div"
		);
	 ...
	 */
	const setupButtonElementCopyToClipboard = (
		button,
		contentElement,
		resultElement,
		contentMaker = undefined
	) =>
	...
	// Export escaped link.
	(() => {
		const linkElement = /** @type {HTMLAnchorElement} */ document.getElementById(
			"export-escaped-anchor"
		);
		const copyButton = /** @type {HTMLButtonElement} */ document.getElementById(
			"export-link-copy-button"
		);
		const copyResultElement = /** @type {HTMLElement} */ document.getElementById(
			"export-link-copy-result"
		);
		...
		revision.latestActiveCommit().subscribe(jsonText => {
			let escapedUrlHash =
				"#" + Config.URL_CIRCUIT_PARAM_KEY + "=" + encodeURIComponent(jsonText);
			linkElement.href = escapedUrlHash;
			linkElement.innerText =
				document.location.href.split("#")[0] + escapedUrlHash;
		});
	})();

	// Export JSON.
	(() => {
		const jsonTextElement = /** @type {HTMLPreElement} */ document.getElementById(
			"export-circuit-json-pre"
		);
		const copyButton = /** @type {HTMLButtonElement} */ document.getElementById(
			"export-json-copy-button"
		);
		const copyResultElement = /** @type {HTMLElement} */ document.getElementById(
			"export-json-copy-result"
		);
		setupButtonElementCopyToClipboard(
			copyButton,
			jsonTextElement,
	...
	// Export final output.
	(() => {
		const outputTextElement = /** @type {HTMLPreElement} */ document.getElementById(
			"export-amplitudes-pre"
		);
		const copyButton = /** @type {HTMLButtonElement} */ document.getElementById(
			"export-amplitudes-button"
		);
		const copyResultElement = /** @type {HTMLElement} */ document.getElementById(
			"export-amplitudes-result"
		);
		const excludeAmps = /** @type {HTMLInputElement} */ document.getElementById(
			"export-amplitudes-use-amps"
		);
		...
		setupButtonElementCopyToClipboard(
			...
			() => {
				let raw = JSON.stringify(
					mostRecentStats.get().toReadableJson(!excludeAmps.checked),
					null,
					" "
				);
				return raw
	...
	// Export offline copy.
	(() => {
		const downloadButton = /** @type {HTMLButtonElement} */ document.getElementById(
